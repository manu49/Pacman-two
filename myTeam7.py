      ...
      for food in protect:
        for enemy in enemies:
          ...
          if dist < min_dist:
            min_dist = dist
            target = food
      features['target'] = self.getMazeDistance(currPos, target)
  ...
  def getFeatures(self,gameState,action):
    ...
    if(num_of_invaders == 0):
      ...
      if((index<2) and ip):
        ...
        targeted_capsule = m if len(dist_capsules)>0 else 0

        #nearest_ghost = min(ghost_distances)
        features['epp'] = targeted_capsule if number_of_ghosts>0 and targeted_capsule < min(ghost_distances) else 0
    ...
    if prevState.numCarrying > 0 and presentState.numCarrying != 0:
      ...
      features['ret'] = min([self.getMazeDistance(presentState.getPosition(), point) for point in border_openings])
