...
class myCustomAgent(CaptureAgent):
    ...
    def chooseAction(self, gameState):
        ...
        values = [int(self.evaluate(gameState, a)) for a in actions]
        maxValue = max(values)
    ...
    def eatFood(self, gameState, action):
        ...
        if switch == 0.0:
            self.collectedFoods = 0.0
        features['collectedFood'] = int(self.collectedFoods) * (min([self.distancer.getDistance(myPos, pos) for pos
                 in [(width / 2, i) for i
                     in range(1, int(height)) if not gameState.hasWall(width / 2, i)]])) * switch
